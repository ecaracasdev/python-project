...
def create_excel_sheet(file_data_list, day, temperature, writer):
    ...
    file_df_filtered['log[M]'] = file_df_filtered.apply(lambda row: np.log10(row.concentration_uM), axis=1)
    file_df_filtered['log[Fo-Fi/Fi]'] = file_df_filtered.apply(lambda row: np.log10((media_Fo - row.media_Fi)/row.media_Fi), axis=1)
